...
The Cascading Schedule Drivers visual tree displays each changed schedule row once. When
several paths reach the same row, subsequent paths show a shared-issue reference.
Branches sort by unique affected unfinished issues with Jira target dates,
largest first. Counts include qualifying descendants hidden by the display limit.
...
data: `audit-detail.csv` and planned epic/dependency CSVs remain available. Resource-group reports show branches
starting in that group, including downstream rows belonging to other groups.
...
These measurements predate the consolidated Cascading Schedule Drivers section.
Wide-export HTML sizes refer to the overall manager report. Cascade sizes refer
to the earlier cascade section, including its then-present complete detail table. The wide export has
...
The visual-tree limits do not limit CSV audit detail.
