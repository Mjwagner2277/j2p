...
| Microsoft Project shows a Font dialog while j2p is running. | Project may have a stale modal dialog from an older run, or an older j2p build may be installed. | Update to the latest j2p, close the Font dialog, close Project, use Task Manager to end any `WINPROJ.EXE` process if needed, then rerun. |
