...
| The sandbox opens without highlighted cells. | The active Project table may not be `j2p Review`, or Project rejected cell formatting during the run. | Apply the `j2p Review` table in Project and check the manager report for `ProjectReviewTableSetupFailed` or `ProjectCellColoringFailed`. |
