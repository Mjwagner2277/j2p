...
| The terminal says predecessor links were planned, but the Project `Predecessors` column is blank. | Microsoft Project rejected or failed to retain every dependency-write method j2p attempted. | Open `audit-detail.csv` and search for `ProjectDependencyWriteFailed`; the message includes every Project API attempted and the readback value. |
